...
def train(args):
    ...
    if args.testing_model == "dataset":
        ...
        assert os.path.isfile(args.image_path), (
            f"Please verify the input image path: {args.image_path}"
        )
        ...
        print(
            f"Anomaly detection results are saved in {save_path}, with an anomaly of {anomaly_score:.3f} "
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_name", type=str, default="test.png", help="Model for testing (default: 'dataset')"
    )
