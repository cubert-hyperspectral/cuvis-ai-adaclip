"""CIR false-color AdaCLIP example with optimal threshold (two-stage decider).

This script uses the optimal threshold found from all-frame analysis:
- Image-level threshold: 0.0847439244389534 (optimal F1)
- Top-k fraction: 0.001 (0.1% of pixels)
- Quantile: 0.995 (for pixel-level mask)

It:
  * Builds a CuvisPipeline explicitly.
  * Uses LentilsAnomalyDataNode → CIRFalseColorSelector → AdaCLIPDetector.
  * Uses TwoStageBinaryDecider with optimal threshold.
...
def main(**kwargs):
    ...
    logger.info("Image-level threshold: {:.6f} (optimal F1)", image_threshold)
    ...
    # Read optimization flags from config (default to True for optimized performance)
    ...
    # Two-stage decider with optimal threshold
    ...
    pipeline_metadata = PipelineMetadata(
        ...
        description=(
            ...
            "(LentilsAnomalyDataNode → CIRFalseColorSelector → AdaCLIPDetector → TwoStageBinaryDecider)"
        ),
    ...
    pipeline_config_path = pipeline_config_dir / "adaclip_cir_false_color_optimal_threshold.yaml"
    ...
    trainrun_config = TrainRunConfig(
        name="adaclip_cir_false_color_optimal_threshold_cli",
