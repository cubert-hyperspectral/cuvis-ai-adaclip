...
def compute_relative_score_tail_minus_median(anomaly_map: np.ndarray, q_tail: float = 0.999, q_median: float = 0.5) -> float:
...
def compute_relative_score_tail_over_median(anomaly_map: np.ndarray, q_tail: float = 0.999, q_median: float = 0.5, eps: float = 1e-6) -> float:
...
def run_analysis_for_band_selector(
    ...
    band_selector = create_band_selector(selector_type, num_spectral_bands, nir_nm, red_nm, green_nm)
            ...
            def create_input_stream_from_frames():
                ...
                for frame_info in calibration_frames + validation_frames:
                    ...
                    batch = {
                        ...
                        "wavelengths": torch.from_numpy(wl_np.astype(np.int32)).unsqueeze(0).to(device),
                    }
                    ...
                    if not predecessors:
                        ...
                        for parent_node in predecessors:
                            for edge_data in temp_pipeline._graph[parent_node][band_selector].values():
    ...
    for r in calibration_results:
        cal_results_data.append({
            ...
            "relative_score_tail_minus_median": float(r["relative_score_tail_minus_median"]),  # q99.9 - q50
            "relative_score_tail_over_median": float(r["relative_score_tail_over_median"]),  # q99.9 / (q50 + eps)
    ...
    for r in validation_results:
        val_results_data.append({
            ...
            "relative_score_tail_minus_median": float(r["relative_score_tail_minus_median"]),  # q99.9 - q50
            "relative_score_tail_over_median": float(r["relative_score_tail_over_median"]),  # q99.9 / (q50 + eps)
            "pixel_threshold": float(r["pixel_threshold"]) if r["pixel_threshold"] is not None else None,
    ...
    cal_scores_normal = [r["image_score"] for r in calibration_results if not r["frame_info"]["has_stones"]]
    cal_scores_anomaly = [r["image_score"] for r in calibration_results if r["frame_info"]["has_stones"]]
    val_scores_normal = [r["image_score"] for r in validation_results if not r["frame_info"]["has_stones"]]
    val_scores_anomaly = [r["image_score"] for r in validation_results if r["frame_info"]["has_stones"]]
    
    # Use distinct colors and styles for all 4 groups
    ax.hist(cal_scores_normal, bins=20, alpha=0.6, label="Calibration Normal", color="blue", edgecolor="black", linewidth=1.5)
    ax.hist(cal_scores_anomaly, bins=20, alpha=0.6, label="Calibration Anomaly", color="red", edgecolor="black", hatch="///", linewidth=1.5)
    ax.hist(val_scores_normal, bins=20, label="Validation Normal", color="green", edgecolor="darkgreen", histtype="step", linewidth=3, linestyle="-")
    ax.hist(val_scores_anomaly, bins=20, label="Validation Anomaly", color="purple", edgecolor="darkviolet", histtype="step", linewidth=3, linestyle="--")
    ax.axvline(optimal_img_threshold, color="orange", linestyle=":", linewidth=2.5, label=f"Threshold ({optimal_img_threshold:.4f})")
    ...
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    cal_rel_diff_normal = [r["relative_score_tail_minus_median"] for r in calibration_results if not r["frame_info"]["has_stones"]]
    cal_rel_diff_anomaly = [r["relative_score_tail_minus_median"] for r in calibration_results if r["frame_info"]["has_stones"]]
    val_rel_diff_normal = [r["relative_score_tail_minus_median"] for r in validation_results if not r["frame_info"]["has_stones"]]
    val_rel_diff_anomaly = [r["relative_score_tail_minus_median"] for r in validation_results if r["frame_info"]["has_stones"]]
    
    # Use distinct colors and styles for all 4 groups
    ax.hist(cal_rel_diff_normal, bins=20, alpha=0.6, label="Calibration Normal", color="blue", edgecolor="black", linewidth=1.5)
    ax.hist(cal_rel_diff_anomaly, bins=20, alpha=0.6, label="Calibration Anomaly", color="red", edgecolor="black", hatch="///", linewidth=1.5)
    ax.hist(val_rel_diff_normal, bins=20, label="Validation Normal", color="green", edgecolor="darkgreen", histtype="step", linewidth=3, linestyle="-")
    ax.hist(val_rel_diff_anomaly, bins=20, label="Validation Anomaly", color="purple", edgecolor="darkviolet", histtype="step", linewidth=3, linestyle="--")
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(selector_output_dir / "relative_score_tail_minus_median_distribution.png", dpi=150, bbox_inches="tight")
    plt.close()
    
    # Relative score: tail over median (q99.9 / (q50 + eps))
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    cal_rel_ratio_normal = [r["relative_score_tail_over_median"] for r in calibration_results if not r["frame_info"]["has_stones"]]
    cal_rel_ratio_anomaly = [r["relative_score_tail_over_median"] for r in calibration_results if r["frame_info"]["has_stones"]]
    val_rel_ratio_normal = [r["relative_score_tail_over_median"] for r in validation_results if not r["frame_info"]["has_stones"]]
    val_rel_ratio_anomaly = [r["relative_score_tail_over_median"] for r in validation_results if r["frame_info"]["has_stones"]]
    
    # Use distinct colors and styles for all 4 groups
    ax.hist(cal_rel_ratio_normal, bins=20, alpha=0.6, label="Calibration Normal", color="blue", edgecolor="black", linewidth=1.5)
    ax.hist(cal_rel_ratio_anomaly, bins=20, alpha=0.6, label="Calibration Anomaly", color="red", edgecolor="black", hatch="///", linewidth=1.5)
    ax.hist(val_rel_ratio_normal, bins=20, label="Validation Normal", color="green", edgecolor="darkgreen", histtype="step", linewidth=3, linestyle="-")
    ax.hist(val_rel_ratio_anomaly, bins=20, label="Validation Anomaly", color="purple", edgecolor="darkviolet", histtype="step", linewidth=3, linestyle="--")
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(selector_output_dir / "relative_score_tail_over_median_distribution.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    axes[0].plot(cal_fpr_main, cal_tpr_main, label=f"Calibration (AUC={cal_auc_main:.3f})", linewidth=2)
    axes[0].plot(val_fpr_main, val_tpr_main, label=f"Validation (AUC={val_auc_main:.3f})", linewidth=2, linestyle="--")
    ...
    cal_rel_diff_scores = np.array([r["relative_score_tail_minus_median"] for r in calibration_results])
    val_rel_diff_scores = np.array([r["relative_score_tail_minus_median"] for r in validation_results])
    ...
    axes[1].plot(cal_fpr_diff, cal_tpr_diff, label=f"Calibration (AUC={cal_auc_diff:.3f})", linewidth=2)
    axes[1].plot(val_fpr_diff, val_tpr_diff, label=f"Validation (AUC={val_auc_diff:.3f})", linewidth=2, linestyle="--")
    ...
    cal_rel_ratio_scores = np.array([r["relative_score_tail_over_median"] for r in calibration_results])
    val_rel_ratio_scores = np.array([r["relative_score_tail_over_median"] for r in validation_results])
    ...
    axes[2].plot(cal_fpr_ratio, cal_tpr_ratio, label=f"Calibration (AUC={cal_auc_ratio:.3f})", linewidth=2)
    axes[2].plot(val_fpr_ratio, val_tpr_ratio, label=f"Validation (AUC={val_auc_ratio:.3f})", linewidth=2, linestyle="--")
...
@click.option("--quantile", type=float, default=0.995,
              help="Quantile for pixel-level threshold calculation")
@click.option("--output-dir", type=str, default="outputs/two_stage_threshold",
              help="Output directory for results")
@click.option("--test-only/--no-test-only", default=True,
              help="Skip training/validation splits and put frames 0-13 into the test split")
@click.option("--image-score-method", type=click.Choice(["max", "top_k_mean"]), default="top_k_mean",
              help="Method for computing image-level score")
@click.option("--top-k", type=int, default=0,
              help="Top-k pixels (set 0 to use --top-k-fraction of pixels)")
@click.option("--top-k-fraction", type=float, default=0.001,
              help="Fraction of pixels to average when using top_k_mean (0.1% -> 0.001)")
@click.option("--threshold-metric", type=click.Choice(["f1", "youden", "roc_optimal"]), default="f1",
              help="Metric for finding optimal image-level threshold")
@click.option("--use-calibrated-threshold/--no-use-calibrated-threshold", default=False,
...
@click.option("--calibration-normal", type=int, default=5,
              help="Number of normal frames for calibration")
@click.option("--calibration-anomaly", type=int, default=5,
              help="Number of anomaly frames for calibration")
@click.option("--band-selector", type=click.Choice(["cir_false", "supervised_cir", "supervised_full", "all"]), 
              default="all", help="Band selector to use: cir_false, supervised_cir, supervised_full, or all")
def main(**kwargs):
    ...
    logger.info(f"Total frames: {len(all_frames)} (normal: {len(normal_frames)}, anomaly: {len(anomaly_frames)})")
    ...
    validation_frames = normal_frames[calibration_normal:calibration_normal+2] + anomaly_frames[calibration_anomaly:calibration_anomaly+2]
    
    logger.info(f"Calibration: {len(calibration_frames)} frames ({calibration_normal} normal, {calibration_anomaly} anomaly)")
    ...
    if len(all_results) > 1:
        ...
        for r in all_results:
            logger.info(f"  {r['selector_type']:20s} - AUC: {r['cal_auc']:.4f}, F1: {r['cal_f1']:.4f}, Acc: {r['cal_accuracy']:.4f}")
        
        logger.info("\nValidation Set:")
        for r in all_results:
            logger.info(f"  {r['selector_type']:20s} - AUC: {r['val_auc']:.4f}, F1: {r['val_f1']:.4f}, Acc: {r['val_accuracy']:.4f}")
