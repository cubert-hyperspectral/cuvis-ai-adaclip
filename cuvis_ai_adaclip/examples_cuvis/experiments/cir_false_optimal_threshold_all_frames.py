...
def find_optimal_threshold(y_true: np.ndarray, y_scores: np.ndarray, metric: str = "f1") -> tuple[float, float, dict]:
    ...
    if metric == "f1":
        ...
        return float(optimal_threshold), optimal_f1, {
            "f1": optimal_f1,
        ...
        if valid_idx.any():
            ...
            optimal_threshold = thresholds[valid_idx][optimal_idx] if optimal_idx < len(thresholds[valid_idx]) else thresholds[-1]
        ...
        return float(optimal_threshold), optimal_recall, {
            "f1": float(optimal_f1),
            "precision": optimal_precision,
            "recall": optimal_recall,
        }
...
def save_frame_visualization(
    ...
    axes[0, 0].set_title(f"CIR False-Color RGB\nImage Score: {image_score:.6f}\nThreshold: {threshold:.6f}\nGate: {gate_status}", 
    ...
    axes[1, 0].set_title(title, fontweight="bold" if passed_gate else "normal",
    ...
    if gt_mask is not None and passed_gate:
        ...
        overlay_title = f"Overlay (Green=TP, Red=FP, Yellow=FN)\n"
        overlay_title += f"Image Score: {image_score:.6f}\nThreshold: {threshold:.6f}\nGate: {gate_status}"
        if iou is not None:
            overlay_title += f"\nIoU: {iou:.4f}"
    elif gt_mask is not None and not passed_gate:
        # Gate failed, no overlay
        overlay_title = f"Overlay (Gate FAILED - No Mask)\n"
        overlay_title += f"Image Score: {image_score:.6f}\nThreshold: {threshold:.6f}\nGate: {gate_status}"
    else:
        # No GT available
        if passed_gate:
    ...
    axes[1, 1].set_title(overlay_title, fontweight="bold" if passed_gate else "normal",
                        color=gate_color if not passed_gate else "black")
    axes[1, 1].axis("off")
...
@click.option("--threshold-metric", type=click.Choice(["f1", "recall"]), default="f1",
              help="Metric for finding optimal threshold: f1 or recall")
@click.option("--output-dir", type=str, default="outputs/cir_false_optimal_threshold_all_frames",
...
def main(**kwargs):
    ...
    axes[0].hist(anomaly_scores, bins=20, alpha=0.6, label="Anomaly", color="red", edgecolor="black", hatch="///")
    axes[0].axvline(optimal_threshold, color="green", linestyle="--", linewidth=2, label=f"Threshold ({optimal_threshold:.4f})")
    ...
    logger.info(f"Optimal threshold: {optimal_threshold:.6f}")
    logger.info(f"Final metrics - Accuracy: {all_accuracy:.4f}, F1: {all_f1:.4f}, AUC: {all_auc:.4f}")
